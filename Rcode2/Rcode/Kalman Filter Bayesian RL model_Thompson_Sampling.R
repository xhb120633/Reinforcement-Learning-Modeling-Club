...
T_sampling<-function(mean,sd){
  i=length(mean)
  p=rep(0,i)
  for(d in 1:i){
    A=matrix(rep(0,(i-1)*i),nrow=i-1,ncol=i)
    A[,d]=1
    for(m in 1:i){
      if(m<d){
        A[m,m]=-1
    ...
    M<-as.vector(M)
    H=A%*%diag(sd)%*%t(A)
    p[d]=pmvnorm(lower=0,upper=Inf,mean=M,sigma=H)[1]
  }
  return(p)
}
...
for(t in 1:ntrials){
  ...
  s_sd_list[t,]=s_sd
  p_choose=T_sampling(s_mean,s_sd)
...
TS_sim1 <- ggarrange(pic_choice_trials1, pic_accre_trials1, ncol = 2, nrow = 1, labels = c("A", "B"),
                      widths = c(1, 1), heights = 1)

ggsave(filename = "./pic/TS_sim1.png", plot = TS_sim1, 
       width = 11, height = 5.5)
...
ggsave(filename = "./pic/TS_mu_plot.png", plot = mu_plot, 
       width = 9, height = 5.5)
...
ggsave(filename = "./pic/TS_v_plot.png", plot = v_plot, 
       width = 9, height = 5.5)
