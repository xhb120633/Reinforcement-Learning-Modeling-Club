...
for(t in 1:ntrials){
  ...
  tmp_reward=sample(c(100,10), size = 1, replace = T, prob = p_reward[choice[t],])
...
exp_decay_sim1 <- ggarrange(pic_choice_trials1, pic_accre_trials1, ncol = 2, nrow = 1, labels = c("A", "B"),
                     widths = c(1.25, 1), heights = 1)

ggsave(filename = "./pic/exp_decay_sim1.png", plot = exp_decay_sim1, 
       width = 11, height = 5.5)
...
v_plot <- 
  ...
  geom_hline(yintercept = 100*p_reward1[1] + 10*p_reward1[2], size = 0.75, colour = "#f1a340", alpha = 0.5) + 
  geom_hline(yintercept = 100*p_reward2[1] + 10*p_reward2[2], size = 0.75, colour = "#998ec3", alpha = 0.5) +
  scale_x_continuous(limits = c(0, 100), breaks = seq(0, 100, by = 20)) +
  pic_theme

ggsave(filename = "./pic/exp_decay_v_plot.png", plot = v_plot, 
       width = 9, height = 5.5)
